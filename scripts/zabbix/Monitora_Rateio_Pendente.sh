for email in `sudo docker exec mysql /usr/bin/mysql --login-path=local -h mysql -uroot -proot mars -e "select email from painel_rateio_horas_logstash where estatus = 'PENDENTE'" -s -N 2> /dev/null`
do
echo "
...
Foi identificado que você possui rateio(s) de marcação de ponto(s) pendente(s) há mais de uma semana.
